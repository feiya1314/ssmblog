...
;(function($){
	$.fn.jcImgScroll = function(options) {
		var defaults = {
			...
			NumBtn : false,
			setNumBtn : {      //setNumBtn 方法  ，省略了function 关键字
				width : 19,
				height : 18,
				x : 0,
				y : 362
		...
		var options = $.extend(defaults,options);   //options会与defaluts合并，并且options会覆盖原有内容，并返回新的defaluts
		//console.log(this);
		//console.log( this instanceof jQuery);
		return this.each(function() {  //this 指的是  $("#demo") ，一个jquery 对象。function 是对于每个匹配到的元素（document对象）所要执行的函数

			//console.log(this);        
			//console.log( this instanceof jQuery);   //false

			var _SELF = $(this),	   //这里的this是document对象，不是jQuery对象，使用$(document)可以转换为jQuery对象
				_arrData = [],          
				_IDX = 0,
				_CENTER = 0,
				_indexArray=[],
			    $IMGLIST = $("a",_SELF),     //$IMGLIST  变量  $表明此变量是jQuery类型的变量（$非必需）
				$IMGWRAP = $("ul",_SELF),
				...
				setSize = function(Dom,l,t,w,h,z,s){
					Dom.show()
						 .css("z-index",z)   //z-index 元素的堆叠顺序
						 .animate({"top":t,"left":l,"height":h,"width":w},s);//s 是速度
				};

				//alert($IMGLIST.html());
				//console.log();
				//console.log( $IMGLIST instanceof jQuery);
				//console.log($IMGLIST);
				//console.log($IMGWRAP);
				//初始化DOM对象
			...
			object.prototype.Initial = function(callback){
				...
				for(a=0;a<this.length;a++){
					var othis = $IMGWRAPLIST.eq(a),    //li中的第a个元素
						olist = othis.find("a"),		//查找"a"元素
					    sPath = olist.attr("path"),		//获取 元素的 两个属性
			...
			_arrData = o.Initial(function(arrImg){
				//console.log(arrImg);
				for(var b in arrImg){   //b 是一个0开始的整数（arrImg的索引）					
					arrImg[b].tmp = b;   //动态地为arrImg添加tmp属性
					arrImg[b].onload = function(){
						var idx = this.tmp,   //arrImg[b]调用的此函数，所以this指的是arrImg[b]
			...
			function imgScroll(index,ArrData,dd,indedArr){
				$IMGWRAPLIST.hide()
							.css("z-index",-1)
							.eq(index)
							.addClass("select")
				...
				var indedArrLen = indedArr.length,i=null;
			...
			var $prev = $("#sPrev",_SELF),
				...
				$dd = $("dd",_SELF);
			console.log(_IDX);
			var centerOfIndex=Math.round(imgLength/2)-1;
			console.log(centerOfIndex);
			$next.click(function(){			
				_indexArray.push(_indexArray.shift());
				_IDX = _IDX -1;
				imgScroll(_IDX,_arrData,$dd,_indexArray);
				if(_IDX==0){
					_IDX=imgLength;
				}
			});
			...
			$prev.click(function(){
					_indexArray.unshift(_indexArray.pop());
					_IDX = _IDX + 1;
					imgScroll(_IDX,_arrData,$dd,_indexArray);
					if(_IDX==imgLength-1){
						_IDX-=imgLength;
